...
class RubricCalculator extends React.Component {
  ...
  get_score(position) {
    ...
    for(var i=0; i<clicks.length; i++) {
      for(var j=0; j<clicks[i].length; j++) {
        if(clicks[i][j] == true) { scores.push(`${i}${j}`) }
      }
    }
  ...
  submitGrade() {
    let isValid = ((this.state.studentName != '') && (this.state.studentID != '') && (this.props.crn != '') && (this.state.currentAttribute != '') && (this.state.scores != [])) ? true : false
    if (isValid) {
      this.gradeService.submitGrade(this.state.studentName, this.state.studentID, this.props.crn, this.state.currentAttribute, this.state.scores)
      this.setState({ showAlert: true }); setTimeout(this.reset, 3000)
    }
  }

  reset() {
    //reset calculator component state and form component state
    this.setState({ text: '', currentAttributePath: '', currentAttribute: '', attributeHasChanged: false, scores: [], clicks: undefined, showAlert: false })
  }
    ...
    rawFile.onreadystatechange = () => {
      if (rawFile.readyState === 4) {
        if (rawFile.status === 200 || rawFile.status == 0) {
          ...
          */
          let max_row = 0
          for(var i=0; i<file_text.length; i++) { if (file_text[i] != '') { max_row += 1 } }
  ...
  renderRow(rowNum, rowContent) {
    ...
    return (
      <div className='rubricRow' key={rowNum}>
        {row.map(function(section) { return section })}
      </div>
    )
  }
  ...
  renderSection(key, value, position) {
    ...
    return ( <RubricSection section_callback={this.get_score} key={key} value={value} position={position} clicked={clicked} /> )
  }
  ...
  render() {
    ...
    if (clicks != undefined) {
      for (var rowNum in file_text) { if (file_text[rowNum] != '') {
        rows.push(this.renderRow(rowNum, file_text[rowNum]))
      }}
    }

    let allContent = (rows.length > 0) ? rows.map(function(row) { return row } ) : null
    ...
    let student_scores = this.state.scores.map(function(score) { return (<h4 className='studentScore' key={`score${score[0]}${score[1]}`}>Row: {score[0]} | Score: {score[1]}</h4>) })
        ...
        <div className='rubric'>
          <div className='rubricHeader'>
            <div className='rubricHeaderSection'>
              <h3 className='rubricHeaderSectionText'>Objectives: Students Will</h3>
            </div>
            <div className='rubricHeaderSection'>
              <h3 className='rubricHeaderSectionText'>Does not meet expectations</h3>
            </div>
            <div className='rubricHeaderSection'>
          ...
          <div className='footerButtons'>
            <button className='submitGrade' onClick={this.submitGrade}>Submit Student Grade</button>
            <button className='resetRubric' onClick={() => this.reset()}>Reset All Fields</button>
          </div>
