...
class RubricSection extends React.Component {
	constructor(props) {
		...
		this.state = {
			clicked:
				this.props.clicked == false || this.props.clicked == true
					? this.props.clicked
					: false,
		};
	...
	componentWillReceiveProps(nextProps) {
		if (this.state.clicked != nextProps.clicked) {
			this.setState({
				clicked:
					nextProps.clicked == false || nextProps.clicked == true
						? nextProps.clicked
						: false,
			});
		}
		...
		if (column_number == 0) {
			...
			return (
				<div className='rubricSectionFinalColumn'>
					<p
						className='rubricSectionFinalColumnText'
						style={{ backgroundColor: bgColor, color: textColor }}
						onClick={this.sectionClicked}
					>
						{this.props.value}
