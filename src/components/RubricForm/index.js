...
class RubricForm extends React.Component {
  ...
  send_child_data() {
    if ((this.state.studentName != '') && (this.state.studentID != '') && (this.state.currentAttributePath != '')) {
      let data = [ this.state.studentName, this.state.studentID, this.state.currentAttribute, this.state.currentAttributePath ]
  ...
  renderMenuSection(file_path, field_name) {
    return (<div onClick={() => this.change_attribute(field_name, file_path)} key={field_name}>{field_name}</div>)
  }
    ...
    for (var i=0; i<(this.state.attributes.length); i++) {
      for(var j=0; j<text_files.length; j++) {
        if (this.state.attributes[i] == text_files[j][1]) {
          menu_content.push(this.renderMenuSection(text_files[j][0], text_files[j][1]))
        }
      }
    }
    let all_menu_content = menu_content.map(function(section) { return section } )
      ...
      <div className='rubricForm'>
        ...
        <div className='rubricInputsDiv'>
          <div className='rubricInputDiv'>
            ...
            <input className='rubricInputTextField' name='studentName' onChange={this.handleInputChange} value={this.state.studentName}></input>
          </div>
          <div className='rubricInputDiv'>
            <h3 className='rubricInputText'>Enter Student ID</h3>
            <input className='rubricInputTextField' name='studentID' onChange={this.handleInputChange} value={this.state.studentID}></input>
          </div>
        </div>
        ...
        <div className='rubricFormAttributeDiv'>
          <h4 className='rubricFormAttribute'>Attribute : {this.state.currentAttribute}</h4>
        </div>

        <div className='rubricFormButtonDiv'>
          <div className='buttonCenteringDiv'>
            <button className='rubricFormSubmit' onClick={this.send_child_data}>Submit</button>
          </div>
        </div>
